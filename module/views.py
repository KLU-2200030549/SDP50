...
def FeedbackFun(request):
    if request.method == 'POST':
        form = FeedbackFormFunction(request.POST)
        if form.is_valid():
            ...
            feedback = FeedbackList(name=name, email=email, phonenumber=phonenumber, comments=comments)
            feedback.save()
            send_mail(
    ...
    if request.method == "POST":
        ...
        if doctorpass1 == doctorpass2:
            if User.objects.filter(username=doctorusername).exists():
                ...
                user.save()
                messages.info(request, 'Account created successfully!!')
                return HttpResponse('homepage_doctor')
        else:
            messages.info(request, 'Password do not match')
            return render(request, 'signup_doctor.html')
